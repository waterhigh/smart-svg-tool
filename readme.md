...
这是一个基于 AI 的智能图片处理工具，能够利用 **Segment Anything Model (SAM)** 进行语义分割，通过 **PaddleOCR** 自动抹除图片文字，并使用 **VTracer** 将位图转换为高质量的 SVG 矢量图。前端提供了一个强大的 **Fabric.js** 画布，支持对拆解后的矢量模块进行自由组合、缩放和导出。

## ✨ 主要功能

* **AI 智能拆解**：点击图片任意位置，利用 SAM 模型自动识别主体并抠图。
* **文本自动抹除**：上传图片时自动调用 PaddleOCR 识别文字，并利用图像修复技术生成无文字的背景底图。
